...
- `style(readme)`: shrink the `graderunner.png` logo in `README.md` — switched from plain Markdown image syntax (which renders at full native resolution, no size control) to an `<img width="320">` tag, since GitHub renders a safe HTML subset in READMEs. Per feedback that the full-size image felt overwhelming at the top of the file.
